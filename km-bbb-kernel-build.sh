...
dpkg -s libncurses-dev > /dev/zero
if [ $? -eq 0 ]; then
    echo "libncurses-dev Package  is installed!"
else
    echo "libncurses-dev Package  is NOT installed!"
...
if [ -f .config ] ; then
	...
	echo "${Purple}make ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- km_bbb_defconfig${NC}"
	make ARCH=arm CROSS_COMPILE=arm-linux-gnueabihf- km_bbb_defconfig
        if [ -f .config ] ; then
