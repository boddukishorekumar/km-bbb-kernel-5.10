...
while [ ! -z "$1" ] ; do
	...
	--mmc)
		...
		# DTB tar file - 5.10.168-Kernel-Masters+-dtbs.tar.gz
		echo -e "${Purple} tar -xvf out/${KERNEL_UTS}-dtbs.tar.gz  -C /mnt/rootfs/boot/dtbs/ ${NC}"
		sudo tar -xvf ./out/${KERNEL_UTS}-dtbs.tar.gz  -C /mnt/rootfs/boot/dtbs/

		echo -e "${Purple} echo uname_r=${KERNEL_UTS} > /mnt/rootfs/boot/uEnv.txt ${NC}"
		echo uname_r=${KERNEL_UTS} > /mnt/rootfs/boot/uEnv.txt

		echo -e "${Purple} tar -xvf out/${KERNEL_UTS}-modules.tar.gz  -C /mnt/rootfs/usr/ ${NC}"
		sudo tar -xvf ./out/${KERNEL_UTS}-modules.tar.gz  -C /mnt/rootfs/usr/
